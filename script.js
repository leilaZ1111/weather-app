...
async function checkweather(city) {
  ...
  if (response.status == 404) {
    ...
    let weekDay = new Date(date).toLocaleDateString("en-US", {
      weekday: "long",
    });
    ...
    document.querySelector(
      ".weekDay"
    ).textContent = `${weekDay}, ${day}th ${longMonthName} ${year}`;
...
async function checkforcast(city) {
  ...
  if (response.status == 404) {
    ...
    data.list.forEach((element) => {
      ...
      if (
        !fiveDayAccurate.includes(
          newDate.toLocaleString("en-us", { weekday: "short" })
        )
      ) {
        fiveDayAccurate.push(
          newDate.toLocaleString("en-us", { weekday: "short" })
        );
      }
    });
    ...
    data.list.forEach((elementDate) => {
      ...
      let newDatex = new Date(elementDate.dt_txt).toLocaleString("en-us", {
        weekday: "short",
      });
